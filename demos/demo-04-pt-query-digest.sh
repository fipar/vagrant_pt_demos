...
pause_msg "We'll first enable slow log and produce some traffic with sysbench"
...
set -x
pt-query-digest --since="$since" --until="$until" --limit 10 --shorten=2048 $slow_log > $report_dest
set +x
